...
# phan chia data train va test 
# o day lay 25 % cho test 
...
# xay dung mo hinh du bao 
# ( so luong layer , ham activation , loi he thong)
...
# khai bao cho mo hinh , o day hidden layer ,
spfnet = create_spfnet([32, 64,128,256], 'relu', 'normal')
spfnet.summary()
...
# bat dau tinh sai so o day 
RMSE_orig = mean_squared_error(y_pred_orig, y_test_orig, squared=False)
print(RMSE_orig)

rmae = np.mean(np.abs((y_pred_orig - y_test_orig) / y_test_orig)) 
print("rmse",rmae)
...
for i in range(100):
  ...
  spfnet.predict(X_train[0,:].reshape(1,-1))
...
# test_image = np.expand_dims(X_test, axis=0).astype(input_details["dtype"])
