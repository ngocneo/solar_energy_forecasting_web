...
# reconstructed_model = keras.models.load_model("my_h5_model_lstm_h1.h5")
...
tflite_model_defaults = tflite_models_dir/"tflite_model_defaults.tflite"
...
for i in range(len(X_test)):
    ...
    # print(predict_test)
    # exit()
    output = run_tflite(tflite_model_defaults,predict_test)
...
testPredict = Maxmin.inverse_transform(y_pred)
y_test = Maxmin.inverse_transform(y_test)
# calculate root mean squared error
trainScore = mean_squared_error(y_test, testPredict)
print('Train Score: %.2f RMSE' % (trainScore))
